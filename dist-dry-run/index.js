...
// src/config/index.ts
var ACCESS_TOKEN_EXPIRES_IN = 7200;
var REFRESH_TOKEN_EXPIRES_IN = 30 * 24 * 3600;
...
authRouter.post("/register", async (c) => {
  ...
  const db2 = getDB();
  const existing = await db2.prepare("SELECT id FROM users WHERE email = ?").bind(email).first();
  if (existing) {
...
authRouter.post("/refresh", async (c) => {
  ...
  ).bind(rt.user_id).first();
  if (!user) {
    return c.json({ error: "invalid_token" }, 401);
  }
  await db2.prepare("DELETE FROM refresh_tokens WHERE token = ?").bind(refresh_token).run();
  const secret = c.env.JWT_SECRET;
...
authRouter.post("/logout", async (c) => {
  ...
  const db2 = getDB();
  await db2.prepare("DELETE FROM refresh_tokens WHERE token = ?").bind(parsed.data.refresh_token).run();
...
userRouter.put("/me", requireAuth, async (c) => {
  ...
  const body = await c.req.json();
  const parsed = external_exports.object({
    current_password: external_exports.string().min(1).optional(),
    new_password: external_exports.string().min(6).optional(),
  ...
  if (new_password) {
    ...
    await db2.prepare("UPDATE users SET password_hash = ? WHERE id = ?").bind(newHash, userId).run();
  }
  if (email && email !== user.email) {
    ...
    await db2.prepare("UPDATE users SET email = ? WHERE id = ?").bind(email, userId).run();
  }
...
userRouter.delete("/me", requireAuth, async (c) => {
  const userId = c.get("userId");
  const db2 = getDB();
  await db2.prepare("DELETE FROM refresh_tokens WHERE user_id = ?").bind(userId).run();
  await db2.prepare("DELETE FROM subscriptions WHERE user_id = ?").bind(userId).run();
  await db2.prepare("DELETE FROM users WHERE id = ?").bind(userId).run();
  return c.body(null, 204);
});

// src/routes/subscriptions.ts
var subscriptionsRouter = new Hono2();
subscriptionsRouter.get("/", requireAuth, async (c) => {
  ...
  if (results.length === 0) {
    results = [{
      ...
      expire_time: Math.floor(Date.now() / 1e3) + 30 * 24 * 3600,
      created_at: Math.floor(Date.now() / 1e3) - 5 * 24 * 3600
    }];
  }
  return c.json({
...
subscriptionsRouter.post("/", requireAuth, async (c) => {
  ...
  const parsed = external_exports.object({
    ...
    traffic_total: external_exports.number().optional(),
    expire_time: external_exports.number().optional()
  }).safeParse(body);
  if (!parsed.success) {
    return c.json({ error: "invalid_request", details: parsed.error.issues }, 400);
  }
...
subscriptionsRouter.put("/:id", requireAuth, async (c) => {
  ...
  const db2 = getDB();
  const sub = await db2.prepare("SELECT id FROM subscriptions WHERE id = ? AND user_id = ?").bind(id, userId).first();
  if (!sub) {
    return c.json({ error: "not_found" }, 404);
  }
  if (parsed.data.name) {
    await db2.prepare("UPDATE subscriptions SET name = ? WHERE id = ?").bind(parsed.data.name, id).run();
  }
  const updated = await db2.prepare(
...
subscriptionsRouter.delete("/:id", requireAuth, async (c) => {
  ...
  await db2.prepare("DELETE FROM subscriptions WHERE id = ?").bind(id).run();
  return c.body(null, 204);
});
subscriptionsRouter.post("/:id/usage", requireAuth, async (c) => {
  ...
  const body = await c.req.json();
  const parsed = external_exports.object({
    upload_add_download: external_exports.number().int().min(0)
  }).safeParse(body);
  if (!parsed.success) {
    return c.json({ error: "invalid_request", details: parsed.error.issues }, 400);
  }
  ...
  const sub = await db2.prepare("SELECT id FROM subscriptions WHERE id = ? AND user_id = ?").bind(id, userId).first();
  if (!sub) return c.json({ error: "not_found" }, 404);
  await db2.prepare("UPDATE subscriptions SET used_traffic = ? WHERE id = ?").bind(parsed.data.upload_add_download, id).run();
  const updated = await db2.prepare(
